...
thmWF_LEX_DEPENDENT = cacheProof "thmWF_LEX_DEPENDENT" ctxtWF .
  prove [txt| !R:A->A->bool S:A->B->B->bool. WF(R) /\ (!a. WF(S a)) ==>
                WF(\(r1,s1) (r2,s2). R r1 r2 \/ (r1 = r2) /\ S r1 s1 s2) |] $
    _REPEAT tacGEN `_THEN` tacREWRITE [defWF] `_THEN` tacSTRIP `_THEN`
    tacX_GEN [txt| P:A#B->bool |] `_THEN` 
    tacREWRITE [thmLEFT_IMP_EXISTS] `_THEN`
    tacGEN_REWRITE id [thmFORALL_PAIR] `_THEN` 
    _MAP_EVERY tacX_GEN [[txt| a0:A |], [txt| b0:B |]] `_THEN` tacDISCH `_THEN`
    _FIRST_X_ASSUM (tacMP . ruleSPEC [txt| \a:A. ?b:B. P(a,b) |]) `_THEN`
    tacREWRITE [thmLEFT_IMP_EXISTS] `_THEN`
    _DISCH_THEN (tacMP . ruleSPECL [[txt| a0:A |], [txt| b0:B |]]) `_THEN` 
    tacASM_REWRITE_NIL `_THEN`
    _DISCH_THEN (_X_CHOOSE_THEN [txt|a:A|] (_CONJUNCTS_THEN2 tacMP tacASSUME)) 
    `_THEN` _DISCH_THEN (tacX_CHOOSE [txt| b1:B |]) `_THEN`
    _FIRST_X_ASSUM (tacMP . ruleSPECL [ [txt| a:A |]
                                      , [txt| \b. (P:A#B->bool) (a,b) |] ])
    `_THEN` tacREWRITE [thmLEFT_IMP_EXISTS] `_THEN`
    _DISCH_THEN (tacMP . ruleSPEC [txt| b1:B |]) `_THEN` 
    tacASM_REWRITE_NIL `_THEN`
    _DISCH_THEN (_X_CHOOSE_THEN [txt| b:B |] 
                   (_CONJUNCTS_THEN2 tacMP tacASSUME)) `_THEN`
    tacDISCH `_THEN` tacEXISTS [txt| (a:A, b:B) |] `_THEN` 
    tacASM_REWRITE_NIL `_THEN`
    tacREWRITE [thmFORALL_PAIR] `_THEN` tacASM_MESON_NIL

thmWF_IND ::  WFCtxt thry => HOL cls thry HOLThm
thmWF_IND = cacheProof "thmWF_IND" ctxtWF .
    prove [txt| WF(<<) <=> !P:A->bool. 
                (!x. (!y. y << x ==> P(y)) ==> P(x)) ==> !x. P(x) |] $
      tacREWRITE [defWF] `_THEN` tacEQ `_THEN` tacDISCH `_THEN` tacGEN `_THEN`
      _POP_ASSUM (tacMP . ruleSPEC [txt| \x:A. ~P(x) |]) `_THEN`
...
thmWF_MEASURE = cacheProof "thmWF_MEASURE" ctxtWF .
    prove [txt| !m:A->num. WF(MEASURE m) |] $
...
thmMEASURE_LE = cacheProof "thmMEASURE_LE" ctxtWF .
    prove [txt| (!y. MEASURE m y a ==> MEASURE m y b) <=> m(a) <= m(b) |] $
...
thmWF_FALSE = cacheProof "thmWF_FALSE" ctxtWF .
    prove [txt| WF(\x y:A. F) |] $ tacREWRITE [defWF]
